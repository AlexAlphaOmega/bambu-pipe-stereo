...
workflow PREPARE_INPUT_STANDARD {
    ...
    // extract spatial coordinates for visium / stereoseq chemistries; add placeholder for others
    ch_unique_spatial = ch_unique_chem.filter { chem -> chem.startsWith('visium') || chem == 'stereoseq' }
    ch_unique_non_spatial = ch_unique_chem.filter { chem -> !chem.startsWith('visium') && chem != 'stereoseq' }
    EXTRACT_10X_SPATIAL_COORDINATES(ch_unique_spatial, ch_barcode_coordinate_config)
    ch_spatial_coordinates = EXTRACT_10X_SPATIAL_COORDINATES.out.spatial_coordinates
        .mix(ch_unique_non_spatial.map { chem -> [chem, null] })
