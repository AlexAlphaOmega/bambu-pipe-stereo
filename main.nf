...
include { PREPROCESS_STEREOSEQ } from './modules/preprocess_stereoseq.nf'
...
workflow {
    ...
    // process standard fastq samples (10x/visium)
    ch_preprocess_fastq_in = ch_standard_fastq.map { sample, path, meta -> [sample, path, meta, meta.barcode] }
    PREPROCESS_FASTQ(ch_preprocess_fastq_in, ch_flank_seq_config, ch_adapter_seq_config)

    // process stereoseq fastq samples
    ch_preprocess_stereo_in = ch_stereoseq_fastq.map { sample, path, meta -> [sample, path, meta] }
    PREPROCESS_STEREOSEQ(ch_preprocess_stereo_in, ch_flank_seq_config)

    // Alignment: mix standard + stereoseq preprocessed FASTQs
    ch_all_fastq = PREPROCESS_FASTQ.out.fastq.mix(PREPROCESS_STEREOSEQ.out.fastq)
    ALIGNMENT(ch_all_fastq, ch_genome, ch_annotation)
    ...
    if (!params.bam_only) {
        ...
        ch_rds_files_collect = BAMBU_CONSTRUCT_READ_CLASS.out.rds
            ...
            .map { collected_tup ->
                ...
                def has_spatial = metas.any { meta -> meta.chemistry.startsWith('visium') || meta.chemistry == 'stereoseq' }
